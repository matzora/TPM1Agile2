...
.. code-block:: php
    ...
    $dql = "SELECT a, e, r FROM Bug a JOIN a.engineer e JOIN a.reporter r ORDER BY a.created DESC";
...
.. code-block:: php
    ...
    $dql = "SELECT a, e, r, p FROM Bug a JOIN a.engineer e ".
           "JOIN a.reporter r JOIN a.products p ORDER BY a.created DESC";
    $query = $entityManager->createQuery($dql);
...
.. code-block:: php
    ...
    $dql = "SELECT a, e, r FROM Bug a JOIN a.engineer e JOIN a.reporter r ".
           "WHERE a.status = 'OPEN' AND (e.id = ?1 OR r.id = ?1) ORDER BY a.created DESC";
...
.. code-block:: php
    ...
    $dql = "SELECT p.id, p.name, count(a.id) AS openBugs FROM Bug a ".
           "JOIN a.products p WHERE a.status = 'OPEN' GROUP BY p.id";
